...
class ConfigManager:
    ...
    def __init__(self, config_file=None):
        ...
        self.load_config()
        # O carregamento de credenciais é feito em main.py, considerando .env
...
# Função validate_config removida por não estar em uso.
# A validação ocorre implicitamente ao buscar valores com get_value e usar defaults.
    ...
    def get_value(self, section: str, key: str, default: Any = None, value_type: type = None) -> Any:
        ...
        try:
            ...
            # Tenta obter a chave como fornecida e em minúsculas
            value = self.config[section].get(key, None) # Tenta a chave original primeiro
            if value is None:
                 value = self.config[section].get(key.lower(), default) # Tenta a chave minúscula
            
            if value is None:
                return default
                
            # Removido lower() global para preservar case de senhas
            str_value = str(value).lower()
            
            if key == 'auto_switch_modes':
                return str_value in ('true', 'yes', '1', 'sim', 'verdadeiro')
...
# Método is_auto_switch_enabled removido (estava comentado).
# A lógica de auto_switch é verificada diretamente onde necessário.
...
# Função load_credentials removida pois as credenciais são carregadas em main.py
# que também considera o arquivo .env sobrepondo o config.ini se necessário.
